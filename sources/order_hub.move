module ilayer::order_hub {
    ...
    public struct OrderHub has key {
        ...
        balances: Table<ID, vector<TokenBalance>>,
    }
    ...
    public fun create_order<T>(
        ...
        // Mark nonce as used
        mark_nonce_used(hub, user, types::order_request_nonce(&request));
    ...
    fun mark_nonce_used(
        ...
        if (!table::contains(&hub.request_nonces, user)) {
            table::add(&mut hub.request_nonces, user, table::new<u64, bool>(ctx));
        };
    ...
    fun process_deposits<T>(
        ...
        // For simplicity, assuming single coin deposit
        // In production, would handle multiple tokens
        
        let balance = coin::into_balance(payment);
        
        if (!table::contains(&hub.balances, order_id)) {
            table::add(&mut hub.balances, order_id, vector::empty());
        };
        
        let balances = table::borrow_mut(&mut hub.balances, order_id);
    ...
    fun return_tokens(
        ...
        // Return all tokens to user
        if (table::contains(&hub.balances, order_id)) {
            let balances = table::remove(&mut hub.balances, order_id);
            
            let i = 0;
            while (i < vector::length(&balances)) {
                let TokenBalance { token_type, balance } = vector::pop_back(&mut balances);
